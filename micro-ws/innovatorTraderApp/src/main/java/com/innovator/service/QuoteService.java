...
import com.innovator.domain.QuoteMapper;
import com.innovator.domain.YahooQuote;
import com.innovator.domain.YahooQuoteResponses;
...
public class QuoteService {
	...
	protected String yahoo_url;// = "https://query.yahooapis.com/v1/public/yql?q=select * from
								// yahoo.finance.quotes where symbol in ('{symbol}')&format={fmt}&env={env}";
	...
	public List<Quote> getQuotes(String symbols) throws SymbolNotFoundException {
		...
		/*
		 * YahooQuoteResponses responses = restTemplate.getForObject(yahoo_url,
		 * YahooQuoteResponses.class, symbols, FMT, ENV); logger.debug("Got responses: "
		 * + responses); List<YahooQuote> yahooQuotes =
		 * responses.getResults().getQuoteList().getQuote(); Date createDate =
		 * responses.getResults().getCreated();
		 * 
		 * 
		 * 
		 * List<Quote> quotes = yahooQuotes.stream() .map(yQuote ->
		 * QuoteMapper.INSTANCE.mapFromYahooQuote(yQuote, createDate))
		 ...
		 * "Timestamp":"Wed Oct 23 13:39:19 UTC-06:00 2013", "MSDate": 41570.568969907,
		 * "MarketCap":476497591530, "Volume":397562, "ChangeYTD":532.1729,
		 * "ChangePercentYTD":-1.44368493773359, "High":52499, "Low":519.175,
		 * "Open":519.175 }
		 */
	...
	/*
	 * public List<Quote> getQuotes(String symbols) throws SymbolNotFoundException {
	 * logger.debug("retrieving quotes for: " + symbols); if ( symbols.isEmpty() )
	 * return new ArrayList<>();
	 * 
	 * YahooQuoteResponses responses = restTemplate.getForObject(yahoo_url,
	 * YahooQuoteResponses.class, symbols, FMT, ENV); logger.debug("Got responses: "
	 ...
	 * List<Quote> finalQuotes= new ArrayList<Quote>();
	 * 
	 * for (Quote quote : quotes) { if ( quote.getName() != null ) {
	 * finalQuotes.add(quote); } } return finalQuotes; }
	 */
