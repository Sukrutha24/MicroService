...
import com.innovator.domain.MarketSummary;
...
public class MarketSummaryService {
	...
	protected void retrieveMarketSummary() throws SymbolNotFoundException {
		 ...
		 */
		// List<Quote> quotesIT =
		// pickRandomThree(Arrays.asList(symbolsIT.split(","))).parallelStream().map(symbol
		// -> getQuote(symbol)).collect(Collectors.toList());
		// List<Quote> quotesFS =
		// pickRandomThree(Arrays.asList(symbolsFS.split(","))).parallelStream().map(symbol
		// -> getQuote(symbol)).collect(Collectors.toList());

		// List<Quote> quotesFS =
		// pickRandomThree(Arrays.asList(symbolsFS.split(","))).stream().map(symbol ->
		// marketService.getQuote(symbol)).collect(Collectors.toList());
		summary.setTopGainers(getTopThree(symbolsIT));
		summary.setTopLosers(getTopThree(symbolsFS));
	}

	/**
	 * Retrieve the list of top winners/losers. Currently retrieving list of 3
	 * random.
	 * 
